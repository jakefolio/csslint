...
CSSLint.addRule({
    ...
    init: function(parser, reporter) {
        ...
        function findLastSemicolon(event) {
            var stream = this._tokenStream;

            // If no property token was stored
            if (lastPropertyTokens.length === 0) {
                return;
            }

            if (!rule.evaluate(stream, lastPropertyTokens.pop())) {
